...
'''
Class that represents a 4 digit 7-segment display PyTell from Didel (http://www.didel.com)
...
class PyTell():
   ...
   The decimal points has weight 128.
    ...
    def __init__(self, i2c_address = 0x20):
        ...
        try:
            if GPIO.RPI_REVISION > 1:
                ...
                PyTell.debug("I2C at bus 1 detected")
            else:
                self._bus = SMBus(0) # For revision 1 Raspberry Pi
                PyTell.debug("I2C at bus 0 detected")
        except:
            PyTell.debug("Failed to detect I2C bus.")
    ...
    def getId(self):
        ...
        @return: the display's id; None if display is not ready or I2C transfer failed
        '''
    ...
    def readByte(self):
        ...
        @return: one byte reply; None if display not ready or I2C transfer failed
        '''
        if not self._isReady:
            return None
        try:
            rc = self._bus.read_byte(self.i2c_address)
            return rc
        except:
           return None
    ...
    def readData(self, cmd):
        ...
        @return: reply as list; None if display not ready or I2C transfer failed
        '''
        if not self._isReady:
            return None
        try:
            rc = self._bus.read_block_data(self.i2c_address, cmd)
            return rc
        except:
           return None
    ...
    def writeData(self, data):
        ...
        if not (type(data) == list or type(data) == tuple)  or len(data) != 4:
            raise Exception("Error in PyTell.writeData():\nWrong parameter type.")
        for v in data:
            if not (type(v) == int):
                raise Exception("Error in PyTell.writeData():\nWrong parameter type.")
        try:
    ...
    def showText(self, text, pos = 0, dp = [0, 0, 0, 0]):
        ...
        '''
        PyTell.debug("showText(" + str(text) + "," + str(pos) + "," + str(dp) + ")")
        if not self._isReady:
            return False
        ...
        for i in range(len(text)):
            try:
                self._text[i] = PyTell.PATTERN[text[i]]
            except:
...
class TickerThread(Thread):
    ...
    def run(self):
        PyTell.debug("TickerThread started")
        ...
        if self._isRunning: # terminated by number of count
            while time.time() - startTime < 2 and self._isRunning:
                PyTell.delay(10)
            self._display.clear()
        PyTell.debug("TickerThread terminated")
        self._isAlive = False
...
class BlinkerThread(Thread):
    ...
    def run(self):
        PyTell.debug("BlinkerThread started")
        nb = 0
        self._isRunning = True
        while self._isRunning:
        ...
        while time.time() - startTime < 1:
            PyTell.delay(1)
        PyTell.debug("BlinkerThread terminated")
        self._isAlive = False
